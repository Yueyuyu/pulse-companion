...
- 2026-09-22 托盘/应用图标沿用 Pulse 原版圆环与脉冲线，只做 Windows 小尺寸适配；源 SVG、ICO 和来源说明在 `assets/icons/`。托盘静态、按主任务栏 DPI 选帧，不混同桌面每应用的品牌/机器人偏好；常规构建不引入图像工具依赖。
...
- 2026-09-23 整个胶囊超过 6px 才拖动，轻点仍执行原按钮操作，拖后不误点击。自动贴边默认关闭，详情与托盘开关同步并保存至 `pulse-window.json`，保存失败回滚；手动贴边不受开关影响。屏幕保护只保留侧轨至少 16×24 CSS px 可抓取区域，不再限制整个详情面板必须完整在屏内。
