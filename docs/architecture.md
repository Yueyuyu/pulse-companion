...
整个胶囊（包括图标、环和数字）共享 6px 拖动阈值；低于阈值保留点击，跨过阈值仅调用一次原生移动，隔离拖后 click。renderer 先提交解除贴边后的布局，再进入系统移动循环，结束由宿主回传 `drag-ended`，期间暂停悬停/失焦收起。快速松手时不再进入迟到的原生移动循环。

`PulseWindowPlacement` 按侧轨所在显示器工作区保留至少 16×24 CSS px 抓取范围，不把详情面板强制塞回屏内。允许部分越界，开合不改侧轨位置。`AutoDock` 兼容旧 Version 1 设置，缺失时默认 false；详情及托盘开关写入 `pulse-window.json`，失败回滚，重启保持。关闭只取消拖到边缘后的自动吸附，显式「贴边收起」仍可用。
