...
默认安装构建 Lab 的共享 renderer，并部署到 `%LOCALAPPDATA%\CodexDesktopCompanion\pulse\<部署 ID>`，Startup 使用 `--live --background`。Windows 登录驻留、随 Codex 显隐，无独立任务栏入口；保留额度、任务、关注、通知和跳转。真实接入仅 Codex，当前版本 `1.2.0` 尚未发布。
...
根目录 `start-pulse.ps1 -InspectWindow` 会切换已识别伴侣进程来检查真实窗口，日常不要将它当作无副作用的示例命令。根目录 `start.ps1 -Development` 仍指向旧 WinForms 实现，不是 Pulse。
